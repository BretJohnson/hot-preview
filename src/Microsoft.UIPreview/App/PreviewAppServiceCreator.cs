...
{
    public static IPreviewAppService Create()
    {
        ...
        foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
        {
            foreach (CustomAttributeData attr in assembly.GetCustomAttributesData())
            {
                ...
                if (appService is not IPreviewAppService previewAppService)
                {
                    throw new InvalidOperationException($"{appServiceType.FullName} doesn't implement {typeof(IPreviewAppService).FullName}");
                }
