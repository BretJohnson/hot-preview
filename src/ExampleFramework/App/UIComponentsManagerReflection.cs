...
public class UIComponentsManagerReflection : UIComponentsManagerBase<UIComponentReflection, ExampleReflection>
{
    ...
    /// UI component / previews can be defined explicitly, using the [Example] attribute, or
    ...
    public void AddUIComponentsFromAssembly(Assembly assembly)
    {
        ...
        foreach (Type type in types)
        {
            ...
            ExampleAttribute? typePreviewAttribute = type.GetCustomAttribute<ExampleAttribute>(false);
            if (typePreviewAttribute != null)
            {
                AddPreview(new ExampleClassReflection(typePreviewAttribute, type));
            }

            MethodInfo[] methods = type.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly);
            foreach (MethodInfo method in methods)
            {
                ExampleAttribute? previewAttribute = method.GetCustomAttribute<ExampleAttribute>(false);

                if (previewAttribute != null)
                {
                    AddPreview(new ExampleStaticMethodReflection(previewAttribute, method));
                }
            }
            ...
            if (CanBeAutoGeneratedPreview(type))
            {
                ...
                if ((uiComponent == null || uiComponent.Previews.Count == 0) && CanBeAutoGeneratedPreview(type))
                {
                    ...
                    var preview = new ExampleClassReflection(type, isAutoGenerated: true);
                    AddPreview(preview);
                }
            }
        }
    ...
    public void AddPreview(ExampleReflection preview)
    {
